...
class ImagePicker(private val context: Context) {
    ...
    fun startActivity(activity: Activity, OnPermissionCallback: OnPermissionCallback) {
        if (isPermissionGranted(context)) {
            activity.startActivityForResult(getIntent(), REQUEST_PICK_IMAGE)
        } else {
    ...
    fun startActivity(fragment: Fragment, onPermissionCallback: OnPermissionCallback) {
        if (isPermissionGranted(context)) {
            fragment.startActivityForResult(getIntent(), REQUEST_PICK_IMAGE)
    ...
    companion object {
         ...
         const val REQUEST_CHECK_PERMISSION = 1134
         const val REQUEST_PICK_IMAGE = 1121
         const val TAG = "SimpleImagePicker"
